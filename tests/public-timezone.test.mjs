...
test("public dashboard renders operating dates in Beijing time", async () => {
  ...
  assert.match(script, /latestWebsiteObservation\(site.id\)/);
  ...
  assert.match(html, /data-view="playbooks" data-index="11"/);
  assert.ok(html.indexOf('data-view="activity"') < html.indexOf('class="nav-archive"'));
