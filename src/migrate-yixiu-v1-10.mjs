...
const at = "2026-09-07T18:21:49.000Z";
...
await store.mutate((state) => {
  ...
  Object.assign(yixiu, {
    ...
    app_store_version: "1.10",
    app_store_release_date: at,
    updated_at: at,
  });

  const eventId = "event-yixiu-v1-10-live-20260908";
  state.events = state.events.filter((event) => event.id !== eventId);
  state.events.unshift({
    id: eventId,
    type: "app_store_release",
    app_id: "yixiu-meditation",
    entity_id: "yixiu-v1-10",
    at,
    source: "official_apple_itunes_lookup_api",
    payload: { version: "1.10", url: appStoreUrl },
  });

  const detectionId = "operational-change-yixiu-v1-10-live-20260908";
  ...
  state.detections.unshift({
    ...
    label: "Yixiu v1.10 is live on the App Store",
    label_zh: "一休冥想 v1.10 已在 App Store 上线",
    current_value: "1.10",
    previous_baseline: "1.9",
    ...
    direction: "neutral",
    detected_at: at,
    evidence: "Apple's official lookup returns version 1.10, the public product URL and the current release date.",
    evidence_zh: "Apple 官方 Lookup API 已返回版本 1.10、公开产品地址与当前发布日期。",
    source_reference: sourceReference,
    verification_type: "api_verified",
  });

  const auditId = "audit-yixiu-v1-10-public-version-20260908";
  state.audit = state.audit.filter((item) => item.id !== auditId);
  state.audit.unshift({
    id: auditId,
    at: "2026-09-09T00:30:00.000Z",
    actor: "AI COO OS",
    app_id: "yixiu-meditation",
    source: "08_30_heartbeat + official_apple_itunes_lookup_api",
...
console.log("YIXIU_PUBLIC_VERSION_OK version=1.10 source=api_verified");
