...
readonly PUBLIC_COMMIT="ffa0a4dc8ebf4fd9ab67c242f56370257b6e5257"
...
readonly TEMP_DIR="$(mktemp -d /tmp/ai-coo-ga4-20260905.XXXXXX)"
readonly BACKUP_DIR="/srv/wonderelian/backups/ops-ga4-20260905-$(date +%Y%m%d%H%M%S)"
...
printf '%s  %s\n' \
  "74667a2741f45bb99f59f8f901b2fcae70311b71c333828fa61f8b27fdca5bf2" "$TEMP_DIR/state.json" \
  "a5a42483064ce188308624dbaa9fea9b57f5935b2804782486a675b61162ffa9" "$TEMP_DIR/brief.json" \
  "a5b489f41c7ba8c80ab4f9f6e5f7b3cc8cb7b0e7ef2b04aff0a4f357b0cbd99b" "$TEMP_DIR/data-health.json" \
  | sha256sum -c -
...
latest = next(row for row in brief["daily_portfolio"]["days"] if row["date"] == "2026-09-05")
...
assert state["metadata"]["data_through"]["website_analytics"] == "2026-09-05"
assert state["metadata"]["data_through"]["app_store"] is None
assert brief["daily_portfolio"]["latest_date"] == "2026-09-05"
...
print("JSON_OK date=2026-09-05 websites=7 content=145 yixiu=13/46/16/null app_store=null")
PY
...
grep -q '"website_analytics": "2026-09-05"' "$ROOT/data/state.json"
grep -q '"latest_date": "2026-09-05"' "$ROOT/data/brief.json"
grep -q '"active_users": 41' "$ROOT/data/brief.json"
echo "DEPLOY_OK_GA4_20260905_FFA0A4D"
