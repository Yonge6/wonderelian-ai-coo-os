...
readonly PUBLIC_COMMIT="0924346c528e119b15d8cf877a7b26cab5213407"
...
readonly TEMP_DIR="$(mktemp -d /tmp/ai-coo-ga4-20260904.XXXXXX)"
readonly BACKUP_DIR="/srv/wonderelian/backups/ops-ga4-20260904-$(date +%Y%m%d%H%M%S)"
...
printf '%s  %s\n' \
  "8fbb8faab8c0cef2fa4a1b99863da02d5d319f995ace9a68bd75c54ccb5cc9e0" "$TEMP_DIR/state.json" \
  "25df3ffc9497e7ae8ce1cac971976f71b724310b87bb469763b5b63992981c89" "$TEMP_DIR/brief.json" \
  "a00168455ea53ccd24e964926768209db458688080123463a5766f244ecc4c47" "$TEMP_DIR/data-health.json" \
  | sha256sum -c -
...
latest = next(row for row in brief["daily_portfolio"]["days"] if row["date"] == "2026-09-04")
...
required_url = "https://www.pinterest.com/pin/1147643917690853905/"
...
assert required_url in content_urls
...
print("JSON_OK date=2026-09-04 websites=7 content=145 yixiu=13/19/16/null app_store=null")
PY
...
echo "DEPLOY_OK_GA4_20260904_0924346"
