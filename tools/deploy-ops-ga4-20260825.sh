...
readonly PUBLIC_COMMIT="3505c5e9940bf9d529537eb098f2de621c7f306f"
...
readonly TEMP_DIR="$(mktemp -d /tmp/ai-coo-ga4-20260825.XXXXXX)"
readonly BACKUP_DIR="/srv/wonderelian/backups/ops-ga4-20260825-$(date +%Y%m%d%H%M%S)"
...
printf '%s  %s\n' \
  "9c7f4f3044a04f1d0d4e24d187e77a35153ab71c4afc10a8da0933a9cd362ed4" "$TEMP_DIR/state.json" \
  "cad0ebda8948edf9e623e105a19a7da507ed3bd3ac1234f87a4108b6c02042ca" "$TEMP_DIR/brief.json" \
  "33d720aee8910c80dcab848641a735d69c8831296c914091a8b1ab6ed20bee15" "$TEMP_DIR/data-health.json" \
  | sha256sum -c -
...
latest = next(row for row in brief["daily_portfolio"]["days"] if row["date"] == "2026-08-25")
yixiu_content = [
    row for row in state["content"]
    if row.get("app_id") == "yixiu-meditation" and row.get("published_at") == "2026-08-25"
]

assert len(state["websites"]) == 7
assert state["metadata"]["data_through"]["website_analytics"] == "2026-08-25"
assert state["metadata"]["data_through"]["app_store"] is None
assert brief["daily_portfolio"]["latest_date"] == "2026-08-25"
...
print("JSON_OK date=2026-08-25 websites=7 yixiu_content=17 app_store=null")
PY
...
grep -q '"website_analytics": "2026-08-25"' "$ROOT/data/state.json"
grep -q '"latest_date": "2026-08-25"' "$ROOT/data/brief.json"
grep -q 'yixiu-youtube-rain-window-sleep-short-20260825' "$ROOT/data/state.json"
echo "DEPLOY_OK_GA4_20260825_3505C5E"
